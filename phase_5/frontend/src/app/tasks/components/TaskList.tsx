...
export default function TaskList() {
  ...
  const { isConnected } = useWebSocket({
    ...
    onTaskUpdate: (event) => {
      console.log('📡 Task update received:', event.event_type);
      ...
      switch (event.event_type) {
        ...
        case 'task.updated':
          setTaskList(prev => prev.map(task =>
            task.id === parseInt(event.task_id)
              ? { ...task, ...event.updated_fields }
              : task
          ));
          break;
        case 'task.completed':
    ...
    <div className="min-h-screen flex">
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="fixed top-4 right-4 z-50">
          {isConnected ? (
            <div className="flex items-center gap-2 px-3 py-2 bg-green-100 text-green-700 rounded-lg shadow-sm">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
              <span className="text-sm font-medium">Live</span>
            </div>
          ) : (
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-100 text-gray-600 rounded-lg shadow-sm">
